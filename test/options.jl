...
@testset "Fides options" begin
    ...
    opt = FidesOptions(maxiter = 100, fatol = 1e-6, frtol = 1e-5, gatol = 1e-3, maxtime = 10.0,
        grtol = 1e-2, verbose = "warning", stepback_strategy = "refine", subspace_solver = "full",
        delta_init = 2.0, mu = 0.30, eta = 0.8, theta_max = 0.99, gamma1 = 0.20, gamma2 = 1.9)
    ...
    @test opt.gamma1 == 0.20
    @test opt.gamma2 == 1.9
    # Test options are properly converted to Python
    opt_py = Fides._fides_options(opt)
    @test pyconvert(Int64, opt_py["maxiter"]) == 100
    @test pyconvert(Float64, opt_py["fatol"]) == 1e-6
    @test pyconvert(Float64, opt_py["frtol"]) == 1e-5
    @test pyconvert(Float64, opt_py["gatol"]) == 1e-3
    @test pyconvert(Float64, opt_py["gatol"]) == 1e-3
    @test pyconvert(Float64, opt_py["maxtime"]) == 10.0
    @test pyconvert(Float64, opt_py["grtol"]) == 1e-2
    ...
    sol = solve(fides_prob, Fides.BFGS(); options = FidesOptions(fatol = 0.0, frtol = 0.0, gatol = 0.0))
    @test sol.retcode == :GTOL
    sol = solve(fides_prob, Fides.BFGS(); options = FidesOptions(stepback_strategy = "refine"))
