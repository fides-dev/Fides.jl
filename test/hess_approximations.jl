...
function test_hess_approximation(prob, hess_approximation; tol_fmin = 1.0e-8, tol_xmin = 1.0e-4)
...
fides_prob = FidesProblem(
    rosenbrock, rosenbrock_grad!, [2.0, 2.0]; lb = [-10.0, -10.0],
    ub = [10.0, 10.0]
)
...
@testset "Hessian approximations" begin
    ...
    test_hess_approximation(fides_prob, Fides.DFP(); tol_fmin = 1.0e-3, tol_xmin = 1.0e-1)
    test_hess_approximation(fides_prob, Fides.BB(); tol_fmin = 1.0e-3, tol_xmin = 1.0e-1)
    test_hess_approximation(fides_prob, Fides.BG(); tol_fmin = 1.0e-3, tol_xmin = 1.0e-1)
